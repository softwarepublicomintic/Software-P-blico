...
function zen_c4c_preprocess_html(&$variables, $hook) {
  ...
  */
  ...
  if ($node = menu_get_object()) {
   $variables['classes_array'][] = drupal_html_class('node-page-'.str_replace(' ','-',strtolower($node->title)));
...
function zen_c4c_preprocess_page(&$variables, $hook) {
  ...
  //if (drupal_match_path(current_path(), 'block/*/edit')) {
    //$title = drupal_get_title();
    //$title_parts = explode(':', $title);
    //$title_fixed = (is_array($title_parts) && isset($title_parts[0]) && isset($title_parts[1])) ? $title_parts[0] . ': ' . t(filter_xss($title_parts[1])) : $title_parts;
    //drupal_set_title($title_fixed);
  //}
}
...
function zen_c4c_date_popup_process_alter(&$element, &$form_state, $context) {
  ...
  */
  ...
  if (isset($element['time'])) {
    $element['time']['#attributes']['class'][] = 'date-time';
  }
  $element['date']['#description'] = 'dd/mm/aaaa';
  if (isset($element['time']) && isset($element['time']['#description'])) {
    $element['time']['#description'] = 'HH:MM';
  }
}
...
function zen_c4c_form_alter(&$form, &$form_state, $form_id) {
  $allowed_forms = array('event_node_form', 'project_node_form', 'project_application_node_form', 'eck__entity__form_edit_managed_slideshow_standard_slideshow', 'bean_form');
